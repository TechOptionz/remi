...
function Layers() {
  return (
    <figure className="eit-layers" aria-label="Moving outward until compassion becomes available">
      ...
      <figcaption className="ladder-cap">Most people need about three layers. The furthest I have gone is five.</figcaption>
    </figure>
  );
}

// THE WAY IN CAN BEGIN BY MOVING FURTHER OUT + COMPASSION RETURNS THROUGH EVERY LAYER
export default function EitOutward() {
  return (
    <Part n={3}>
      <H3 v={['left', 'rule']}>The way in can begin by moving further out</H3>
      <div className="dsr-grid">
        <div className="triad-prose">
          ...
          <Body>There is no prize for reaching compassion quickly. The movement outward is not avoidance of the intervention. It is the intervention respecting the person’s existing route to safety.</Body>
        </div>
        <Layers />
      </div>

      <H3 v={['left', 'rule']}>Compassion returns through every layer</H3>
      <div className="triad-prose">
        <Body>Once compassion becomes available, we reverse direction. I ask the client to bring that compassion one step inward to the part that could not access it before. Inevitably, when the distance has been sufficient and the compassion is genuine, they can. Then we bring it through the next layer, and the next, until the client can feel compassion for the original frustration, numbness, dissociation or bodily sensation.</Body>
